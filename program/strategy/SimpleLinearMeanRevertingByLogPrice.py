...
model0 = PandasRollingOLS(y=np.log(y), x=np.log(x), window=lookback)
# data = data[lookback-1:]
betas = model0.beta
data['betas'] = betas

data['portfolio_position'] = data.apply(lambda x: np.log(x[y_ticker]) - x['betas'] * np.log(x[x_ticker]), axis=1)
# yport = pd.DataFrame(data[y_ticker] - data['betas'] * data[x_ticker])
...
if __name__ == '__main__':
    ...
    test = TearsheetStatistic(data, title='Simple Linear Mean Reverting by log price Strategy')
    test.save()